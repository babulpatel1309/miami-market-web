...
export default async function HotPlate() {
  ...
  const hotPlate = content.hot_plate;
  const soupSizes = content.menu.soup_sizes;
  const menuPdfUrl = content.site.menu_pdf_url;

  const sideChips = soupSizes.map((s, i) => (i === 0 ? `Sides · ${s}` : s));
  const [badge1, badge2] = hotPlate.rotating_badges.length >= 2 ? hotPlate.rotating_badges : ["ALWAYS", "HOT PLATE"];
    ...
    >
      ...
      <div className="animate-reveal-view relative z-2">
        ...
        <h2 className="mt-3.5 font-bricolage text-[clamp(32px,4.5vw,54px)] leading-[0.98] font-extrabold tracking-tight text-green-dark">
          {hotPlate.heading_line1} <span className="text-accent">{hotPlate.heading_accent}</span>
        </h2>
        <p className="mt-5 max-w-[42ch] text-lg leading-relaxed text-text-muted">{hotPlate.body}</p>
        ...
        >
          Download This Month&apos;s Menu (PDF)
        </a>
      </div>
      ...
      <div className="animate-reveal-view relative z-2 px-[18px] pt-[30px]">
        <div className="relative mx-auto max-w-[430px] -rotate-[2.5deg] rounded bg-white px-3.5 pt-3.5 pb-[50px] shadow-[0_34px_64px_-26px_rgba(20,61,34,0.55)]">
          ...
          <img
            src={hotPlate.image_url ?? "/images/hotplate-menu.jpeg"}
