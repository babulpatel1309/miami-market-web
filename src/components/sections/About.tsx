...
export default async function About() {
  const about = (await getWebsiteContent()).about;
  ...
  return (
    <section className="mx-auto grid max-w-[1240px] grid-cols-[repeat(auto-fit,minmax(300px,1fr))] items-center gap-12 px-6 pt-24 pb-[70px]">
      ...
      <div className="animate-reveal-view">
        ...
        <p className="mt-5 text-lg leading-relaxed text-text-muted">{about.paragraph1}</p>
        <p className="mt-3.5 text-lg leading-relaxed text-text-muted">{about.paragraph2}</p>
      </div>
    </section>
  );
}
