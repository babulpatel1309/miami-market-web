...
import { mapServerStatus, useOpenStatus } from "@/hooks/useOpenStatus";
import type { WebsiteHoursCopy, WebsiteScheduleRow, WebsiteStatus, WebsiteStructuredHoursRow } from "@/lib/cms";
...
interface HoursSectionClientProps {
  ...
  initialStatus: WebsiteStatus | null;
}
...
export default function HoursSectionClient({
  ...
  const status = useOpenStatus(structured, initialStatus ? mapServerStatus(initialStatus) : null);
  const signWord = status?.signWord ?? "···";
  const statusLabel = status?.statusLabel ?? "Checking today's hours";
  const statusSub = status?.statusSub ?? "Deli & Drive-Thru 10–8 Mon–Sat";
  const dayName = status?.dayName ?? "Today";
  const todayIdx = status?.todayIdx ?? -1;
  const statusDot = status?.statusDot ?? "#7BD68C";
        ...
        <div className="grid grid-cols-[minmax(280px,0.85fr)_minmax(320px,1.4fr)] items-stretch gap-6">
          ...
          <div className="animate-reveal-view rounded-mm bg-white p-[clamp(20px,2.6vw,30px)] shadow-[0_24px_46px_-26px_rgba(20,61,34,0.5)]">
            ...
            <div className="mt-2 flex flex-col gap-0.5">
              {schedule.map((r) => {
                const today = r.day_of_week === todayIdx;
                return (
                  <div
                    key={r.day_name}
